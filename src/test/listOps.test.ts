...
expect.extend({
	toHaveValues(received: ReturnType<typeof List.create>, ...expected: unknown[]): jest.CustomMatcherResult {
		...
		return {
			pass,
			message: (): string => (pass ? '' : `Expected to see the following values: ${JSON.stringify(expected)}, actual: ${JSON.stringify(values)}`)
		}
	}
})
