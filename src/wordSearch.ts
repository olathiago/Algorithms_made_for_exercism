...
export class WordSearch {
		...
		for (let word of this.words) {
			...
			if (segment.startsWith(word)) {
				this.result[word] = {
					...
					end: [pos[0] + 1 + (word.length - 1) * dir[0], pos[1] + 1 + (word.length - 1) * dir[1]]
				}
				break
			}
		}
	...
	private segment([row, col]: Position, [rowDirection, colDirection]: Direction): string {
		const chars: string[] = []
		while (0 <= row && row < this.grid.length && 0 <= col && col < this.grid[0].length) {
